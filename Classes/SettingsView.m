...
- (UITableViewCell *)newCellForIdentifier:(NSString *)identifier {
	...
	if ([identifier isEqualToString:kIASKPSToggleSwitchSpecifier]) {
		...
		[((IASKSwitchEx *)cell.accessoryView) setOnTintColor:GLACIER_MAIN_COLOR];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if ([cell isKindOfClass:[IASKPSTextFieldSpecifierViewCell class]]) {
		...
		[field setTextColor:GLACIER_MAIN_COLOR];
	}

	if ([cell isKindOfClass:[IASKPSTitleValueSpecifierViewCell class]]) {
		cell.detailTextLabel.textColor = [UIColor grayColor];
	} else {
		cell.detailTextLabel.textColor = GLACIER_MAIN_COLOR;
	}
	return cell;
}
@end
...
INIT_WITH_COMMON_CF {
	[self setTintColor:[GLACIER_MAIN_COLOR adjustHue:5.0f / 180.0f saturation:0.0f brightness:0.0f alpha:0.0f]];
	return self;
}
...
- (NSSet *)findHiddenKeys {
...
#ifndef DEBUG
    if (bctbx_list_size(accounts) >= 1) {
        [hiddenKeys addObject:@"assistant_button"];
    }
    [hiddenKeys addObject:@"settings_group"];
    ...
    [hiddenKeys addObject:@"account_pushnotification_preference"];
#endif
...
- (void)settingsViewController:(IASKAppSettingsViewController *)sender
	...
	if ([key isEqual:@"assistant_button"]) {
        // set the next view to be loginView
