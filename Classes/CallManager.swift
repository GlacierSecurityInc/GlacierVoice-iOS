...
@objc class CallManager: NSObject {
	...
	func callByCallId(callId: String?) -> Call? {
        ...
        if let calltest = calls {
            for acall in calltest {
                if let cid = acall.callLog?.callId {
                    if acall.remoteAddress?.username == callId {
                        return acall
                    }
                }
            }
	...
	@objc func displayIncomingCall(callId: String) {
		...
		if (call != nil) {
            let addr = FastAddressBook.displayName(for: call?.remoteAddress?.getCobject) ?? "Unknown"
			...
			displayIncomingCall(call: nil, handle: "\(callId) Calling", hasVideo: false, callId: callId)
		}
	}
    ...
    @objc func terminateCall(call: OpaquePointer?) { 
        ...
        do {
            ...
            Log.directLog(BCTBX_LOG_ERROR, text: "Failed to terminate call because \(error)")
        }
...
class CoreManagerDelegate: CoreDelegate {
	...
	override func onCallStateChanged(lc: Core, call: Call, cstate: Call.State, message: String) {
		...
		let address = FastAddressBook.displayName(for: addr?.getCobject) ?? "Unknown"
		...
		switch cstate {
			case .IncomingReceived:
				if (CallManager.callKitEnabled()) {
                    ...
                    if (uuid == nil && addr?.username != nil) {
                        uuid = CallManager.instance().providerDelegate.uuids["\(addr!.username)"]
                    }
