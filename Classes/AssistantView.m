...
- (void)viewWillAppear:(BOOL)animated {
	...
	if (!mustRestoreView) {
        ...
        nextView = _remoteProvisioningLoginView; //_loginView;
        if (localLogin) {
            nextView = _loginView;
            localLogin = false;
        }
...
//#if DEBUG
        ...
        if ([view isEqual:_remoteProvisioningLoginView]) {
            ...
            NSString *orgid = [glacierDefaults stringForKey:@"orgid"];
            atf.text = orgid;
...
- (IBAction)onRemoteProvisioningLoginClick:(id)sender {
    ...
    NSString *trimmedOrg = [[self findTextField:ViewElement_Domain].text stringByTrimmingCharactersInSet:
                               [NSCharacterSet whitespaceCharacterSet]];
    [glacierDefaults setObject:trimmedOrg forKey:@"orgid"];
    [glacierDefaults synchronize];
    
    //currently the only supported alternate route for Voice. Can be used on other servers
    // but not recommended.
    if ([trimmedOrg isEqualToString:@"apple"] || [trimmedOrg isEqualToString:@"Apple"]) {
        [glacierDefaults setObject:@"none" forKey:@"connection"]; //means no vpn
        [glacierDefaults synchronize];
        [self setAltRoute];
