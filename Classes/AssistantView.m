...
- (void)viewWillAppear:(BOOL)animated {
	...
	if (!mustRestoreView) {
        ...
        nextView = _remoteProvisioningLoginView;
        if (localLogin) {
            nextView = _loginView;
            localLogin = false;
        }
...
- (void)reset:(BOOL)resetAccounts {
    ...
    if (currentView != nil && currentView != _remoteProvisioningLoginView) {
        nextView = _remoteProvisioningLoginView;
...
+ (NSString *)errorForLinphoneAccountCreatorPasswordStatus:(LinphoneAccountCreatorPasswordStatus)status {
	switch (status) {
		...
		case LinphoneAccountCreatorPasswordStatusInvalidCharacters: /**< Contain invalid characters */
            return @"Invalid password, see password rules.";
...
//#if DEBUG
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:secPath]) {
            ...
            atf.text = [secretDict objectForKey:@"defaultDomainAddress"];
        }
        
        // remoteProvisioningLoginView repurposed as Cognito login
        if ([view isEqual:_remoteProvisioningLoginView]) {
            if (self.cognitoUsername) {
                UIAssistantTextField *cutf =
                    (UIAssistantTextField *)[self findView:ViewElement_Username inView:view ofType:UIAssistantTextField.class];
                cutf.text = self.cognitoUsername;
            }
        }
        
        if ([view isEqual:_remoteProvisioningLoginView] || [view isEqual:_resetPasswordView] || [view isEqual:_forgotPasswordResetView]) {
            UIAssistantTextField *putf =
            (UIAssistantTextField *)[self findView:ViewElement_Password inView:view ofType:UIAssistantTextField.class];
            if (eyeLabel == nil) {
                eyeLabel = [[InsetsLabel alloc] initWithFrame:CGRectMake(0,0,40,20)];
                if (@available(iOS 13.0, *)) {
                    eyeLabel.contentInsets = UIEdgeInsetsMake(0, 0, 0, 10);
                }
                eyeLabel.contentMode = UIViewContentModeCenter;
                eyeLabel.font = [UIFont fontWithName:@"FontAwesome" size:20];
                eyeLabel.textAlignment = NSTextAlignmentCenter;
                eyeLabel.text = @"";
                [eyeLabel setUserInteractionEnabled:YES];
            ...
            if (putf.rightView == nil) {
                putf.rightViewMode = UITextFieldViewModeAlways;
                putf.rightView = eyeLabel;
            }
        }
//#endif
	}
...
- (void)eyeTap
{
    BOOL firstResp = eyeLabel.isFirstResponder;
    if (firstResp) { [eyeLabel resignFirstResponder]; }
    UIAssistantTextField *putf = (UIAssistantTextField *)[self findView:ViewElement_Password inView:currentView ofType:UIAssistantTextField.class];
    if (putf.isSecureTextEntry) {
        eyeLabel.text = @"";
    } else {
        eyeLabel.text = @"";
    }
    [putf setSecureTextEntry:!putf.isSecureTextEntry];
    if ([currentView isEqual:_resetPasswordView]) {
        UIAssistantTextField *putf2 = (UIAssistantTextField *)[self findView:ViewElement_Password2 inView:currentView ofType:UIAssistantTextField.class];
        [putf2 setSecureTextEntry:!putf2.isSecureTextEntry];
    }
    if (firstResp) { [eyeLabel becomeFirstResponder]; }
}
...
- (UIButton *)findUIButton:(ViewElement)tag {
...
- (void)prepareErrorLabels {
	...
	[password showError:[AssistantView
				   ...
				   when:^BOOL(NSString *inputEntry) {
                        if ([inputEntry isEqualToString:@""]) {
                            password.errorLabel.text = @"Password required.";
                            return true;
                        }
                       
                       if (currentView == _resetPasswordView || currentView == _remoteProvisioningLoginView) {
                           return false;
                       }
    ...
    [password2 showError:@"Passwords must match"
					when:^BOOL(NSString *inputEntry) {
                        if ([inputEntry isEqualToString:@""]) {
                            password.errorLabel.text = @"Password confirmation required.";
                            return true;
                        }
					  return false;
					}];
	...
	[domain showError:[AssistantView errorForLinphoneAccountCreatorDomainStatus:LinphoneAccountCreatorDomainInvalid]
				 when:^BOOL(NSString *inputEntry) {
				   if (![inputEntry isEqualToString:@""]) {
                       ...
                       domain.errorLabel.text = @"Domain address required.";
                       if (currentView != nil && currentView == _remoteProvisioningLoginView) {
                           domain.errorLabel.text = @"Org ID required.";
                       }
                   }
				   return true;
				 }];
...
- (void)shouldEnableNextButton {
    ...
    if (currentView != nil && (currentView == _remoteProvisioningLoginView || currentView == _forgotPasswordView || currentView == _forgotPasswordResetView)) {
        [self findUIButton:ViewElement_NextButton].enabled = !invalidInputs;
        if (!invalidInputs) {
            //set background
            [self findUIButton:ViewElement_NextButton].backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"color_Gl.png"]];
            [[self findUIButton:ViewElement_NextButton] setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        } else {
            [self findUIButton:ViewElement_NextButton].backgroundColor = [UIColor clearColor];
            [[self findUIButton:ViewElement_NextButton] setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        }
    }
}
...
- (BOOL) checkFields {
	UISwitch *emailSwitch = (UISwitch *)[self findView:ViewElement_EmailFormView inView:self.contentView ofType:UISwitch.class];
	if (emailSwitch != nil && emailSwitch.isOn) {
        ...
        UIAssistantTextField *pfield = [self findTextField:ViewElement_Password];
        UIAssistantTextField *pfield2 = [self findTextField:ViewElement_Password2];
        if (pfield.text.length < 8) {
            [pfield showError:[AssistantView errorForLinphoneAccountCreatorPasswordStatus:LinphoneAccountCreatorPasswordStatusTooShort]];
            return FALSE;
        } else if (![self isValidPassword:pfield.text]) {
            [pfield showError:[AssistantView errorForLinphoneAccountCreatorPasswordStatus:LinphoneAccountCreatorPasswordStatusInvalidCharacters]];
            return FALSE;
        } else if (![pfield.text isEqualToString:pfield2.text]) {
            pfield2.errorLabel.text = @"Passwords must match";
            return FALSE;
        }
...
-(BOOL)isValidPassword:(NSString *)checkString{
    ...
    NSString *stricterFilterString = @"^(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[0-9])[A-Za-z0-9$@#!%*?&-_]{8,}$";
...
- (BOOL)textField:(UITextField *)textField
	...
	if (textField.tag == ViewElement_SMSCode) {
		// max 6 length
        [self shouldEnableNextButton];
		return range.location + range.length <= 6;
	} else {
		UIAssistantTextField *atf = (UIAssistantTextField *)textField;
		BOOL replace = YES;
...
// when not using the standard Glacier process and closed network
BOOL altRoute = false;
- (void) setAltRoute {
    altRoute = true;
}
...
- (IBAction)onLoginClick:(id)sender {
	ONCLICKBUTTON(sender, 100, {
        ...
        if (altRoute) {
            //break up username into domain and user
            // do I need to change the address used for ping test?
		...
		if (transports || altRoute) {
            NSString *type = @"TLS";
            if (transports) {
                type = [transports titleForSegmentAtIndex:[transports selectedSegmentIndex]];
            }
...
- (IBAction)onRemoteProvisioningLoginClick:(id)sender {
    ...
    // trimming whitespace
    NSString *trimmeduser = [[self findTextField:ViewElement_Username].text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    
    // currently the only supported alternate route for Voice. Can be used on other servers
    // but not recommended.
    if ([trimmeduser hasPrefix:@"90001@"] || [trimmeduser isEqualToString:@"90002@"]) {
        [self setAltRoute];
        [self onLoginClick:sender];
    } else {
        [PhoneMainView.instance handleLoginSelection:YES];
            
        // trimming whitespace
        NSString *trimmedpass = [[self findTextField:ViewElement_Password].text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        self.passwordAuthenticationCompletion.result = [[AWSCognitoIdentityPasswordAuthenticationDetails alloc] initWithUsername:trimmeduser password:trimmedpass];
    }
}
...
-(void) getPasswordAuthenticationDetails: (AWSCognitoIdentityPasswordAuthenticationInput *) authenticationInput  passwordAuthenticationCompletionSource: (AWSTaskCompletionSource<AWSCognitoIdentityPasswordAuthenticationDetails *> *) passwordAuthenticationCompletionSource {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if(!self.cognitoUsername && !ssoUsed)
...
-(void) didCompletePasswordAuthenticationStepWithError:(NSError*) error {
    dispatch_async(dispatch_get_main_queue(), ^{
        if(error){
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:error.userInfo[@"message"] preferredStyle:UIAlertControllerStyleAlert];
...
- (IBAction)onForgotPassClick:(id)sender {
    ONCLICKBUTTON(sender, 100, {
        nextView = _forgotPasswordView;
        [self changeView:nextView back:NO animation:TRUE];
    });
}
...
- (IBAction)onForgotPass2Click:(id)sender {
    ONCLICKBUTTON(sender, 100, {
        ...
        AWSCognitoIdentityUserPool *pool = [AWSCognitoIdentityUserPool defaultCognitoIdentityUserPool];
        AWSCognitoIdentityUser *user = [pool getUser:trimmeduser];
        
        [[user forgotPassword] continueWithBlock:^id _Nullable(AWSTask<AWSCognitoIdentityUserForgotPasswordResponse *> * _Nonnull task) {
            dispatch_async(dispatch_get_main_queue(), ^{
                _waitView.hidden = YES;
                if(task.error){
                    [self alertWithTitle:@"Cannot Reset Password" message:@"There may not be an email or phone associated with this account, you will need to contact Glacier support to reset password."];
                    
                    nextView = _remoteProvisioningLoginView;
                    [PhoneMainView.instance displayAssistantView:NO];
                }else {
                    self.cognitoUsername = trimmeduser;
                    nextView = _forgotPasswordResetView;
                    [self changeView:nextView back:NO animation:TRUE];
                }
            });
            return nil;
...
- (IBAction)onUpdatePasswordClick:(id)sender {
    ONCLICKBUTTON(sender, 100, {
        ...
        NSString *trimmedcode = [[self findTextField:ViewElement_SMSCode].text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        NSString *trimmednewpass = [[self findTextField:ViewElement_Password].text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        
        AWSCognitoIdentityUserPool *pool = [AWSCognitoIdentityUserPool defaultCognitoIdentityUserPool];
        AWSCognitoIdentityUser *user = [pool getUser:self.cognitoUsername];
        
        [[user confirmForgotPassword:trimmedcode password:trimmednewpass] continueWithBlock:^id _Nullable(AWSTask<AWSCognitoIdentityUserConfirmForgotPasswordResponse *> * _Nonnull task) {
            dispatch_async(dispatch_get_main_queue(), ^{
                _waitView.hidden = YES;
                if(task.error){
                    //[self alertWithTitle:@"Cannot Reset Password" message:@"Are you sure you typed the confirmation code correctly and followed password requirements?"];
                    
                    dispatch_async(dispatch_get_main_queue(), ^{
                        UIAlertController * alert = [UIAlertController
                                                     alertControllerWithTitle:@"Cannot Reset Password"
                                                     message:@"Are you sure you typed the confirmation code correctly and followed password requirements?"
                                                     preferredStyle:UIAlertControllerStyleAlert];
                        ...
                        UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"Try again" style:UIAlertActionStyleCancel handler:nil];
...
- (IBAction)onSSOLogin:(id)sender {
    ...
    _waitView.hidden = NO;
    [PhoneMainView.instance handleLoginSelection:NO];
}
...
- (IBAction)onResetPasswordClick:(id)sender {
    if ([self checkFields]) { //repurpose check fields
        _waitView.hidden = NO;
        NSString *trimmedpass = [[self findTextField:ViewElement_Password].text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        self.passwordRequiredCompletionSource.result = [[AWSCognitoIdentityNewPasswordRequiredDetails alloc] initWithProposedPassword:trimmedpass userAttributes:@{}.mutableCopy];
    }
}
...
- (void)didCompleteNewPasswordStepWithError:(NSError * _Nullable)error {
    dispatch_async(dispatch_get_main_queue(), ^{
        if(error){
            // if session expired return to other screen to restart
            NSString *errmsg = error.userInfo[@"message"];
            if ([errmsg hasPrefix:@"Invalid session"]) {
                errmsg = @"You took too long! Starting login process again.";
                sessionTimeout = true; 
            ...
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:errmsg preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction *cancel = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleCancel handler:nil];
            [alert addAction:cancel];
            [self presentViewController:alert animated:YES completion:nil];
            _waitView.hidden = YES;
...
- (void)setNeedsResetPassword {
    nextView = _resetPasswordView;
    [AssistantView cleanTextField:nextView];
    [self changeView:nextView back:FALSE animation:TRUE];
}

- (void)getNewPasswordDetails:(nonnull AWSCognitoIdentityNewPasswordRequiredInput *)newPasswordRequiredInput newPasswordRequiredCompletionSource:(nonnull AWSTaskCompletionSource<AWSCognitoIdentityNewPasswordRequiredDetails *> *)newPasswordRequiredCompletionSource {
    if (!sessionTimeout) {
        self.passwordRequiredCompletionSource = newPasswordRequiredCompletionSource;
        dispatch_async(dispatch_get_main_queue(), ^{
            _waitView.hidden = YES;
            [self setNeedsResetPassword];
        });
    }
}
...
- (void)refreshYourUsername {
	...
	if (uri) {
		_accountLabel.text = [NSString stringWithFormat:NSLocalizedString(@"Your SIP address will be sip:%s@sip.glacier.com", nil), uri]; 
