...
- (void)setAddress:(NSString *)address {
    ...
    NSArray *components = [_addressLabel.text componentsSeparatedByString:@"@"];
    if (components.count == 2) {
        _addressLabel.text = [components firstObject];
    }
...
- (IBAction)onChatClick:(id)event {
    [self goGlacierMessenger];  // go to Glacier Messenger if possible
}

// go to Glacier Messenger if possible
- (void)goGlacierMessenger
{
    ...
    else
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"URL error"
                                                        message:[NSString stringWithFormat:@"No way to get to Glacier Messenger"]
