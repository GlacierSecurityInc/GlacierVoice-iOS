...
- (void)enteringForeground:(NSNotification *)notif {
    [self updateTempStatus:@"Initializing"]; 
    LinphoneProxyConfig *config = linphone_core_get_default_proxy_config(LC);
    [self proxyConfigUpdate:config];
}
...
- (void)proxyConfigUpdate:(LinphoneProxyConfig *)config {
    ...
    if (tempStatus) {
        message = tempStatus;
        if ([tempStatus isEqualToString:@"Initializing"]) {
            message = nil;
        }
    } else if ([PhoneMainView.instance.currentView equal:AssistantView.compositeViewDescription] || [PhoneMainView.instance.currentView equal:CountryListView.compositeViewDescription]) {
        message = nil;
...
- (void) noOpenVPNAlert {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"OpenVPN Connect not installed" message:@"Please download OpenVPN Connect from the App Store." preferredStyle:UIAlertControllerStyleAlert];
