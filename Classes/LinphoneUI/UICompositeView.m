...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *tabBarYConstraint;
...
- (void)update:(UICompositeViewDescription *)description
    ...
    statusBarFrame.size.width = viewFrame.size.width;
    ...
    if (!currentViewDescription.sideMenuEnabled) {
        ...
        if(@available(iOS 13.0, *)) {
            _sideMenuLeadingConstraint.constant = -3 * sideMenuFrame.size.width;
        }
        ...
        if(@available(iOS 13.0, *)) {
            _sideMenuLeadingConstraint.constant = -115;
        }
	...
	if (self.tabBarViewController != nil && currentViewDescription.tabBarEnabled) {
		...
		if (UIInterfaceOrientationIsPortrait([self currentOrientation])) {
            ...
            tabFrame.origin.y = viewFrame.origin.y + viewFrame.size.height - tabFrame.size.height;
		} else {
			tabFrame.origin.y = origin;
			tabFrame.size.height = viewFrame.size.height - tabFrame.origin.y;
		}
        if(@available(iOS 13.0, *)) {
            _tabBarYConstraint.constant = 0;
            _tabBarBottomConstraint.constant = 0;
        }
	} else {
        ...
        if(@available(iOS 13.0, *)) { 
            _tabBarYConstraint.constant = tabFrame.size.height;
            _tabBarBottomConstraint.constant = tabFrame.size.height+tabFrame.size.height;
        }
	}
