...
NSString *const GLACIER_APPLICATION_KEY = @"group.com.glaciersec.apps";
NSString *const GLACIER_ACCT_KEY = @"com.glaciersec.Acct";
...
- (void)setBluetoothEnabled:(BOOL)enable {
	if (_bluetoothAvailable) {
		// The change of route will be done in enableSpeakerWithEnable
...
+ (NSString *)oldPreferenceFile:(NSString *)file {
    ...
    //return [fullPath stringByAppendingPathComponent:file];
    return [self preferenceFile:file];
}
...
- (void)loadAvatar {
	NSString *assetId = [self lpConfigStringForKey:@"voice_settings_avatar.png"];
	...
	if (!ret) {
        ret = [UIImage imageNamed:@"voice_settings_avatar.png"];
	}
	_avatar = ret;
}
