...
- (void)migrationAllPre {
	...
	if ([self lpConfigBoolForKey:@"migration_xmlrpc"] == NO) {
		[self lpConfigSetString:@"https://subscribe.glacier.com:444/wizard.php"
						 forKey:@"xmlrpc_url"
					  inSection:@"assistant"];
		[self lpConfigSetString:@"sip:rls@sip.glacier.com" forKey:@"rls_uri" inSection:@"sip"];
...
- (BOOL)migrateChatDBIfNeeded:(LinphoneCore *)lc {
	...
	if (shouldMigrate) {
		...
		if (!identity) {
			identity = "sip:unknown@sip.glacier.com";
		}
...
- (void)migrationLinphoneSettings {
		...
		while (proxies) {
			...
			if (addr &&
				...
				linphone_proxy_config_set_quality_reporting_collector(
					proxy, "sip:voip-metrics@sip.glacier.com;transport=tls");
	...
	if ([self lpConfigBoolForKey:@"file_transfer_migration_done"] == FALSE) {
		const char *newURL = "https://www.glacier.com:444/lft.php";
...
#pragma deploymate pop
    ...
    if (state == LinphoneCallConnected && pushBgTaskCall) {
        [[UIApplication sharedApplication] endBackgroundTask:pushBgTaskCall];
        pushBgTaskCall = 0;
    }
...
- (BOOL)isConnectingToNetwork {
    return self.connecting;
}

- (void)setConnectingToNetwork:(BOOL)isConnecting {
    self.connecting = isConnecting;
}

- (BOOL)isWaitingToConnect {
    return self.waitingToConnect;
}
	...
	if (theLinphoneCore != nil) {
		...
		if ((flags == 0) || (flags & networkDownFlags)) {
			...
			if (lm.connectivity != newConnectivity) {
                ...
                if ((lm.connectivity == wwan && newConnectivity == wifi) ||
                    (lm.connectivity == wifi && newConnectivity == wwan)) {
                    if (!lm.waitingToConnect) {
                        lm.waitingToConnect = TRUE;
                        [lm performSelector:@selector(tryNetworkReconnect) withObject:nil afterDelay:6];
                    }
                } else {
...
- (void) tryNetworkReconnect {
    if (linphone_core_get_default_proxy_config(LC) && ![self isConnectingToNetwork]) {
        linphone_core_set_network_reachable(theLinphoneCore, false);
        UIApplication *app = [UIApplication sharedApplication];
	...
	if (status == LinphoneAccountCreatorStatusAccountLinked) {
		...
		if (cfg &&
			...
			UIAlertController *errView = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Link your account", nil)
																			 message:[NSString stringWithFormat:NSLocalizedString(@"Link your Glacier account %s to your phone number.", nil),
...
- (bool)allowSpeaker {
    ...
    //this will screw things up if headphones or bluetooth?
...
- (void)call:(const LinphoneAddress *)iaddr {
	...
	if (!iaddr) {
		UIAlertController *errView = [UIAlertController alertControllerWithTitle:@"Invalid address"
																		 message:@"This is likely due to no account configured. Call your Glacier representative for help."
																  preferredStyle:UIAlertControllerStyleAlert];
