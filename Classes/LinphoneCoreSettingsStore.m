...
- (void)transformAccountToKeys:(NSString *)username {
	...
	// default values
	{
        ...
        [self setBool:NO forKey:@"account_watch_notifications_preference"];
        [self setObject:@"" forKey:@"account_external_number_preference"];
        ...
        [self setCString:"*97" forKey:@"voice_mail_uri_preference"];
	}
...
- (void)transformLinphoneCoreToKeys {
	...
	// advanced section
	{
		...
		[self setBool:[lm lpConfigBoolForKey:@"use_rls_presence" withDefault:NO] forKey:@"use_rls_presence"]; 
...
- (BOOL)synchronize {
	@try {
		...
		if ([self integerForKey:@"use_rls_presence"]) {
			...
			NSString *rls_uri = [lm lpConfigStringForKey:@"rls_uri" inSection:@"sip" withDefault:@"sips:rls@sip.glacier.com"];
...
- (void)getExternalPreferences {
    ...
    bool watchPref = [[pdict objectForKey:@"account_watch_notification_preference"] boolValue];
...
- (void)saveExternalPreferences {
    ...
    NSNumber *watchPref = [self objectForKey:@"account_watch_notification_preference"];
    if (!watchPref) {
        watchPref = [NSNumber numberWithBool:NO];
    }
