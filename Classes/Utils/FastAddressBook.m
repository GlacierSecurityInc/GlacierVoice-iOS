...
+ (Contact *)getContactWithDisplayName:(NSString *)displayName {
    Contact *contact = nil;
    if (LinphoneManager.instance.fastAddressBook != nil) {
        NSMutableDictionary *amap = LinphoneManager.instance.fastAddressBook.addressBookMap;
        @synchronized(amap) {
            for(id key in amap)  {
                Contact *curcontact = [amap objectForKey:key];
                if ([curcontact.displayName isEqualToString:displayName]) {
                    contact = curcontact;
                    break;
                }
            }
...
+ (NSString *)displayNameForAddress:(const LinphoneAddress *)addr {
	...
	if (contact && ![contact.displayName isEqualToString:NSLocalizedString(@"Unknown", nil)] && ![contact.displayName isEqualToString:@""])
...
- (void)addContactRecordsWithVCardData:(NSData *)data
{
    CNSaveRequest *saveRequest = [[CNSaveRequest alloc] init];
    
    NSError *error1;
    NSArray *conArray =[CNContactVCardSerialization contactsWithData:data error:&error1];
    if (error1) {
        NSLog(@"Vcard Parse Error: %@", [error1 description]);
        return;
    }
    
    if (LinphoneManager.instance.fastAddressBook != nil) {
        NSMutableArray *dcontacts = [[NSMutableArray alloc] init];
        NSMutableDictionary *amap = LinphoneManager.instance.fastAddressBook.addressBookMap;
        @synchronized(amap) {
            for(id key in amap)  {
                Contact *curcontact = [amap objectForKey:key];
                if ([curcontact.sipAddresses count] > 0) {
                    [dcontacts addObject:curcontact];
                }
            }
        }
        
        for (Contact *contact in dcontacts) {
            [self deleteContact:contact];
        }
    }
    
    // remove in order to re-add
    for (CNContact *contact in conArray) {
        Contact *testContact = [[Contact alloc] initWithCNContact:contact];
        BOOL deleted = FALSE;
        
        if ([[contact phoneNumbers] count] > 0) {
            CNPhoneNumber *cnphoneNum = [[contact phoneNumbers][0] value];
            NSString *phone = cnphoneNum.stringValue;
            LinphoneProxyConfig *cfg = linphone_core_get_default_proxy_config(LC);
            const char *normvalue = linphone_proxy_config_normalize_phone_number(cfg, phone.UTF8String);
            LinphoneAddress *addr = linphone_proxy_config_normalize_sip_uri(cfg, normvalue);
            const char *phone_addr = linphone_address_as_string_uri_only(addr);
            Contact *realContact = [FastAddressBook getContact:[NSString stringWithUTF8String:phone_addr]];
            if (realContact) {
                [self deleteContact:realContact];
                deleted = TRUE;
            }
        }
        
        if (!deleted && testContact.displayName != nil){
            Contact *realContact = [FastAddressBook getContactWithDisplayName:testContact.displayName];
            if (realContact) {
                [self deleteContact:realContact];
            }
        }
    ...
    // added delay or else add doesn't register
    dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, 0.5 * NSEC_PER_SEC);
    dispatch_after(popTime, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
        // add contacts
        for (CNMutableContact *contact in conArray) {
        ...
        @try {
            if([store executeSaveRequest:saveRequest error:&saveError]) {
                NSLog(@"Saved contacts");
                dispatch_async(dispatch_get_main_queue(), ^{
                    for (CNMutableContact *contact in conArray) {
                        Contact *tmpContact = [[Contact alloc] initWithCNContact:contact];
                        [self updateFriend:tmpContact];
                    }
                    [LinphoneManager.instance setContactsUpdated:TRUE];
                });
            }else {
                NSLog(@"Save contacts error : %@", [saveError description]);
                LOGE(@"Save contacts error");
            }
        } @catch (NSException *exception) {
            NSLog(@"=====>>>>> CNContact SaveRequest failed : description = %@", [exception description]);
            LOGE(@"CNContact SaveRequest failed");
        }
