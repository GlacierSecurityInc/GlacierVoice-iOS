...
- (void)addContactRecordsWithVCardData:(NSData *)data
{
    ...
    NSError *error1;
    NSArray *arr =[CNContactVCardSerialization contactsWithData:data error:&error1];
    if (error1) {
    ...
    for (CNContact *contact in arr) {
        ...
        BOOL deleted = FALSE;
        if ([testContact.sipAddresses count] > 0) {
            NSString *extvalue = (NSString *)testContact.sipAddresses[0];
            LinphoneAddress *addr = [LinphoneUtils normalizeSipOrPhoneAddress:extvalue];
            Contact *realContact = [FastAddressBook getContactWithAddress:addr];
            if (realContact) {
    ...
    // add contacts
    for (CNMutableContact *contact in arr) {
