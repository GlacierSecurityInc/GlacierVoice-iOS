...
+ (BOOL)hasSelfAvatar {
	return [NSURL URLWithString:[LinphoneManager.instance lpConfigStringForKey:@"voice_settings_avatar"]] != nil;
}
+ (UIImage *)selfAvatar {
	NSURL *url = [NSURL URLWithString:[LinphoneManager.instance lpConfigStringForKey:@"voice_settings_avatar"]];
...
//normalize number in case of external/contact
+ (LinphoneAddress *)normalizeFromAddr:(const LinphoneAddress *)addr {
    ...
    LinphoneAddress *normaddr = [LinphoneUtils normalizeSipOrPhoneAddress:num_str];
    ms_free(num_address);
...
+ (LinphoneAddress *)normalizeSipOrPhoneAddress:(NSString *)value {
    ...
    //normalize number without + if country code starts with 1
